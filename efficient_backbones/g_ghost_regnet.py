...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = conv3x3(
            width, width, stride, width // min(width, group_width), dilation
        )
...
class Stage(nn.Module):
    def __init__(
        ...
        self.cheap = nn.Sequential(
            nn.Conv2d(cheap_planes, cheap_planes, kernel_size=1, stride=1, bias=False),
            nn.BatchNorm2d(cheap_planes),
            #             nn.ReLU(inplace=True),
        )
...
def regnetx_032(**kwargs):
    return RegNet(
        Bottleneck, [2, 6, 15, 2], [96, 192, 432, 1008], group_width=48, **kwargs
    )
