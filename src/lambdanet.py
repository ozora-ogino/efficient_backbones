...
class LambdaConv(nn.Module):
    def __init__(self, in_channels, out_channels, heads=4, k=16, u=1, m=23):
        ...
        self.kk, self.uu, self.vv, self.mm, self.heads = (
            k,
            u,
            out_channels // heads,
            m,
        ...
        if self.local_context:
            self.embedding = nn.Parameter(
                torch.randn([self.kk, self.uu, 1, m, m]), requires_grad=True
            )
        else:
            self.embedding = nn.Parameter(
                torch.randn([self.kk, self.uu]), requires_grad=True
            )
    ...
    def forward(self, x):
        ...
        if self.local_context:
            ...
            lambda_p = F.conv3d(
                values, self.embedding, padding=(0, self.padding, self.padding)
            )
