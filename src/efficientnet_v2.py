...
def conv_3x3_bn(inp: int, oup: int, stride: int) -> nn.Module:
...
class MBConv(nn.Module):
    def __init__(
        self, inp: int, oup: int, stride: int, expand_ratio: float, use_se: bool
    ):
...
class EffNetV2(nn.Module):
    def __init__(self, cfgs: List[List[int]], width_mult: float = 1.0):
